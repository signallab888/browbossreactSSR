...
export default function Home() {
  ...
  // Brow Try-On state
  const [tryOnOriginal, setTryOnOriginal] = useState<string | null>(null);
  ...
  const handleTryOnUpload = async (file: File) => {
    ...
    try {
      ...
      setTryOnResult(`data:image/png;base64,${data.result}`);
    } catch (err) {
      setTryOnError(err instanceof Error ? err.message : "Could not process photo");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground selection:bg-primary selection:text-primary-foreground">
      ...
      {/* AI Brow Try-On */}
      <section id="try-on" className="py-16 md:py-24 bg-[#F5F1EC] border-t border-zinc-200">
        <div className="container mx-auto px-4">
          <motion.div
            initial="hidden"
            whileInView="visible"
            ...
            className="text-center mb-10 md:mb-14"
          >
            <p className="text-[10px] tracking-[0.38em] uppercase text-zinc-400 mb-3">Powered by AI</p>
            <h2 className="text-4xl md:text-5xl font-serif font-light mb-4">Try Your Perfect Brows</h2>
            ...
            <p className="text-sm text-zinc-500 font-light max-w-sm mx-auto leading-relaxed">
              Upload a selfie and our AI will show you what professionally designed brows would look like on your face — in seconds.
            </p>
          </motion.div>

          <div className="max-w-3xl mx-auto">
            {/* Upload zone — shown when no image loaded yet */}
            {!tryOnOriginal && !tryOnLoading && (
              <motion.div
              ...
              >
                <input
                  ...
                  capture="user"
                  className="hidden"
                  onChange={e => { const f = e.target.files?.[0]; if (f) handleTryOnUpload(f); }}
                />
                <button
                  onClick={() => tryOnInputRef.current?.click()}
                  onDragOver={e => e.preventDefault()}
                  onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) handleTryOnUpload(f); }}
                  className="w-full border-2 border-dashed border-zinc-300 hover:border-black transition-colors duration-300 rounded-none py-16 px-8 flex flex-col items-center gap-5 group cursor-pointer bg-white"
                >
                  <div className="w-16 h-16 rounded-full border border-zinc-200 group-hover:border-black transition-colors flex items-center justify-center">
                    <svg className="w-7 h-7 text-zinc-300 group-hover:text-black transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 16v-8m-4 4l4-4 4 4M20.25 14.15v2.25a2.25 2.25 0 01-2.25 2.25h-12a2.25 2.25 0 01-2.25-2.25v-2.25" />
                    </svg>
                  </div>
                  <div className="text-center">
                    <p className="font-serif text-xl mb-1">Upload Your Selfie</p>
                    <p className="text-xs tracking-[0.2em] uppercase text-zinc-400">Tap to select or drag & drop</p>
                  </div>
                  <p className="text-[10px] text-zinc-300 tracking-wider uppercase">JPG · PNG · HEIC · up to 10MB</p>
                </button>

                <p className="text-center text-[10px] tracking-[0.2em] uppercase text-zinc-400 mt-5">
            ...
            {tryOnLoading && (
              ...
              >
                ...
                {tryOnOriginal && (
                  <img src={tryOnOriginal} alt="Your photo" className="w-24 h-24 object-cover rounded-full border-2 border-white shadow-md grayscale opacity-60" />
                )}
                <div className="text-center">
                  <p className="font-serif text-xl mb-2">Designing Your Perfect Brows</p>
                  <p className="text-xs tracking-[0.2em] uppercase text-zinc-400">This takes about 15–30 seconds…</p>
                </div>
              </motion.div>
            )}

            {/* Result */}
            {tryOnResult && tryOnOriginal && !tryOnLoading && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="grid grid-cols-2 gap-3 md:gap-6 mb-8">
                  <div>
                    <div className="aspect-square overflow-hidden bg-zinc-100 mb-3">
                      <img src={tryOnOriginal} alt="Before" className="w-full h-full object-cover" />
                    </div>
                    <p className="text-center text-[10px] tracking-[0.3em] uppercase text-zinc-400">Before</p>
                  </div>
                  <div>
                    <div className="aspect-square overflow-hidden bg-zinc-100 mb-3 ring-2 ring-black ring-offset-2">
                      <img src={tryOnResult} alt="After — AI designed brows" className="w-full h-full object-cover" />
                    </div>
                    <p className="text-center text-[10px] tracking-[0.3em] uppercase text-zinc-800 font-medium">After</p>
                  </div>
                </div>

                <div className="text-center space-y-4">
                  <p className="font-serif text-lg text-zinc-700">Love what you see?</p>
                  <a
                    href={bookingUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <div>
                    <button
                      onClick={() => { setTryOnOriginal(null); setTryOnResult(null); setTryOnError(null); if (tryOnInputRef.current) tryOnInputRef.current.value = ""; }}
                      className="text-[10px] tracking-[0.25em] uppercase text-zinc-400 hover:text-black transition-colors border-b border-zinc-200 hover:border-black pb-0.5"
                    >
            ...
            {tryOnError && !tryOnLoading && (
              ...
              >
                <p className="text-sm text-zinc-500">{tryOnError}</p>
                <button
                  onClick={() => { setTryOnOriginal(null); setTryOnResult(null); setTryOnError(null); if (tryOnInputRef.current) tryOnInputRef.current.value = ""; }}
                  className="text-[10px] tracking-[0.25em] uppercase text-zinc-400 hover:text-black transition-colors border-b border-zinc-200 hover:border-black pb-0.5"
                >
